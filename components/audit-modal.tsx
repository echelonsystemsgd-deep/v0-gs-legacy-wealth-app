...
export const AuditModal: React.FC = () => {
  ...
  // Map selected tier to prefilled industry or select defaults
  useEffect(() => {
    if (isOpen) {
      setErrorMsg(null)
      setFirstName('')
      ...
      // Set fallback default industry if not set
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Pass along UTM values to Calendly too
    ...
    try {
      ...
      console.error('[AuditModal] Submission failed, redirecting as fallback:', err)
      // Fallback redirection to ensure we do not block user from scheduling
      window.location.href = redirectUrl.toString()
    } finally {
