...
import { Plus, Edit2, Archive, ArchiveRestore, Star, Loader2, MessageSquareQuote, X, Save } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-xxs font-bold uppercase tracking-[0.3em] text-gold/70">Social Proof</p>
          <h1 className="font-serif text-3xl font-bold text-foreground mt-1">Testimonials</h1>
          <p className="text-sm text-muted-foreground mt-1">Manage client feedback shown on the public site.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass rounded-2xl border border-gold/15 p-6 w-full max-w-lg shadow-2xl space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="font-serif text-xl font-bold text-foreground">{modal === 'edit' ? 'Edit Testimonial' : 'Add Testimonial'}</h2>
              <button onClick={() => setModal(null)}><X size={18} className="text-muted-foreground hover:text-foreground" /></button>
            </div>
